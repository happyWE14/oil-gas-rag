...
这是一个面向油气科研论文采集与分析的证据型 RAG 系统。系统将学术 PDF 转换为可检索的文本块、结构化材料记录和实验指标，并让生成结果能够追溯到来源论文及原文证据。

项目采用**Java 大模型应用后端 + 确定性文档工作流**，而不是自主 Agent。后端组合 PostgreSQL/pgvector 检索、Elasticsearch 材料搜索、Redis 协调与缓存、外部论文数据源、PDF 转 Markdown 和大模型结构化抽取，并通过 Vue 前端展示处理过程与结果。

## 项目概览

| 问题 | 说明 |
|---|---|
| 输入是什么？ | 检索条件，以及来自 arXiv、CORE、Semantic Scholar 或本地存储的论文元数据和有权使用的 PDF/Markdown 文档 |
| 系统做什么？ | 相关性筛选 → 下载 → 文档转换 → Markdown 结构感知分块 → Embedding → 单篇论文范围召回 → 两阶段材料与属性抽取 |
| 输出是什么？ | 可搜索的论文库、材料实体、结构化指标与实验条件、来源论文关联、保留的证据片段，以及可选的文档证据问答结果 |
| 后续工程工作增加了什么？ | 可复现的本地运行配置、Elasticsearch 材料索引/搜索/同步、翻译与缓存、论文搜索运行适配、抽取结果同步、失败处理改进、本地文件访问、前端集成和端到端验证 |
| 当前运行效果如何？ | 自定义 RAG 页面已列出 92 篇向量化论文；使用 DashScope Embedding 检索证据并由 DeepSeek 生成回答；8 个离线后端测试、前端类型检查和生产构建均已通过 |

本地运行使用的论文语料、文件、数据库内容和向量数据均未随仓库公开。

## 项目来源与贡献边界

本项目基于一名研究生学长提供的既有后端原型、系统架构、数据库设计和初始论文处理流程继续开发。当前公开仓库是经过清理后重新导入的代码快照，因此导入提交不能解释为整个代码库均由同一人从零编写。

| 范围 | 贡献边界 |
|---|---|
| 继承的项目基础 | 总体架构、数据库设计、初始 Spring Boot 后端，以及原始论文生命周期和任务编排设计 |
| 后续工程工作 | 将继承的系统配置到可复现运行；集成 Elasticsearch 材料索引、结构化搜索、数据同步和相关接口；增加翻译与缓存；适配论文搜索执行流程及外部模型/OCR 配置；扩展结果同步、失败处理、工作流运行行为和本地文件访问；完成 PostgreSQL/pgvector、Redis、Elasticsearch 与模型服务的集成和验证 |
| AI 辅助实现 | 前端及部分代码实现、重构、调试和文档整理大量使用了生成式 AI；需求拆解、代码审查、系统集成、运行调试和验收验证仍由当前项目维护者负责 |

本仓库不将原始架构、数据库设计、后端基线和初始状态机设计声明为后续个人成果。Elasticsearch 文本检索与结构化材料检索已经实现；向量写入和 Hybrid 分数融合仍为部分实现，不作为已完成能力声明。
...
系统在概率性模型调用之外提供了有状态、可重试、可追溯证据的后端约束：处理失败保持可见，抽取结果保留支撑 Chunk，并由确定性工作流规则控制每个模型步骤的执行时机。

## 实际运行效果与复用边界

以下截图均来自实际运行的系统，依次展示运行看板、多源论文搜索配置、单篇论文处理状态、论文与材料库、结构化指标、观察记录、原文证据片段及来源论文关联。后文还展示了通用文档 RAG 的真实检索证据和生成回答。
