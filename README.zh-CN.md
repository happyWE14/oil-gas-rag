...
- **后端：** Java 17、Spring Boot、PostgreSQL/pgvector、Redis 与 Elasticsearch；
- **RAG：** Markdown 结构感知分块、2048 维 Embedding、单篇论文 Top-K 召回，以及携带证据 Chunk 的两阶段结构化抽取；
- **工作流：** 15 状态论文生命周期、有限并发、重试、陈旧任务恢复，以及可选的 Outbox/RocketMQ 路径；
- **实测结果：** 本地自定义 RAG 页面列出 92 篇已向量化论文，并使用 DashScope Embedding 与 DeepSeek 完成一次证据约束问答；语料和向量未收入仓库；
- **检查结果：** 8 个离线后端测试通过，13 个依赖环境的集成测试跳过，前端类型检查与生产构建通过。

## 个人贡献与 AI 辅助说明

本人负责后端系统与 Elasticsearch 接入，包括实现、集成和验证。开发过程中使用 AI 辅助工具进行代码建议、重构、调试和文档整理。Elasticsearch 文本检索与结构化材料检索已经实现；向量写入和 Hybrid 分数融合仍是部分实现，不作为已完成成果声明。
